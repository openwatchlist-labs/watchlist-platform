...
func AcquireLocal(path, sourceURL string, acquiredAt time.Time) (Acquired, error) {
	f, err := os.Open(path)
	if err != nil {
	...
	return makeAcquired(data, sourceURL, AcquisitionLocal, "application/xml", "", "", acquiredAt), nil
}
...
func AcquireHTTP(ctx context.Context, rawURL string, acquiredAt time.Time) (Acquired, error) {
	...
	req.Header.Set("User-Agent", "openwatchlist-platform/ofac-source-ingestor-v0.1.1")
	...
	// Preserve the stable official endpoint in the manifest. The final GovCloud
	// redirect is a short-lived pre-signed URL and must never be persisted.
	return makeAcquired(data, u.String(), AcquisitionHTTP, media, resp.Header.Get("ETag"), resp.Header.Get("Last-Modified"), acquiredAt), nil
}
...
func validateOfficialURL(raw string) (*url.URL, error) {
	...
	if host == officialOFACGovCloudHost {
		...
		if !strings.HasPrefix(cleanPath, "/Published/") || !strings.EqualFold(path.Base(cleanPath), "SDN.XML") {
			return nil, fmt.Errorf("%w: GovCloud redirect path is not an OFAC SDN publication", ErrInvalidSource)
		}
	}
	return u, nil
}
...
func makeAcquired(data []byte, url string, method AcquisitionMethod, media, etag, last string, at time.Time) Acquired {
	if at.IsZero() {
		at = time.Now().UTC()
	} else {
		at = at.UTC()
	...
	return Acquired{Bytes: data, SourceURL: url, Method: method, MediaType: media, ETag: etag, LastModified: last, AcquiredAt: at, ContentSHA256: hex.EncodeToString(sum[:]), ContentLength: int64(len(data))}
}
