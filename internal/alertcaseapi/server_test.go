...
func TestHTTPAlertCaseLifecycle(t *testing.T) {
	...
	h.ServeHTTP(rr, httptest.NewRequest(http.MethodPost, "/v1/alerts", bytes.NewReader(raw)))
	...
	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, httptest.NewRequest(http.MethodPost, "/v1/cases", bytes.NewReader(body)))
	if rr.Code != http.StatusCreated {
	...
	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, httptest.NewRequest(http.MethodPost, "/v1/cases/"+caseResp.Case.CaseID+"/events", bytes.NewReader(body)))
	if rr.Code != http.StatusOK {
		t.Fatalf("case event status=%d body=%s", rr.Code, rr.Body.String())
	}
	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, httptest.NewRequest(http.MethodPost, "/v1/cases/"+caseResp.Case.CaseID+"/verify", nil))
	if rr.Code != http.StatusOK {
...
func TestStrictUnknownField(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	server.Handler().ServeHTTP(rr, httptest.NewRequest(http.MethodPost, "/v1/alerts", bytes.NewBufferString(`{"unknown":true}`)))
