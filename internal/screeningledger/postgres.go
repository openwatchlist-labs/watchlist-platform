...
const (
	// ADR-0007 Addendum 13 D116: these four moved again, one round after
	// D107 moved them -- measured (not guessed) as sha256(prosrc) of
	// 023's literal bodies and SchemaSQL's own copy. F-B's fix drops
	// both overloads' mirror aggregate's ledger_id scoping (now global,
	// matching the eligibility predicate it corroborates) and adds a
	// non-vacuity check that p_ledger_id resolves to a ledger with rows,
	// D87's standing cost (0007:9031-9049), two migrations later.
