...
//
// acceptedBodySHA256 is ADR-0007 Addendum 10 D87/D86 row 8: D77 declared
// the guard TRIGGER functions' bodies and stopped -- these two functions
// are the ones that actually write purged_at, and until this addendum
// nothing declared their body at all, so a T2 substitution was reported
// by no observer (CheckProvisioningState and grant-ddl-ownership both
// clean). D77's own reasoning applies one object over: the live body on
// a migration-bootstrapped database is 020's (019 is superseded by
// 020's CREATE OR REPLACE the moment both run in order), so the
// two-member set is 020's literal plus SchemaSQL's -- not 019's, which
// is never the live body on any bootstrap path this repository ships.
...
var requiredDefinerFunctions = []requiredDefinerFunction{
	{
		name: "screening_ledger_purge_snapshots", identityArgs: "text,int8,timestamptz,text,text",
		installedBy: "db/migrations/022_screening_ledger_purge_chain_corroboration.sql",
		acceptedBodySHA256: []string{
	...
	{
		name: "screening_ledger_purge_snapshots", identityArgs: "text[],text,int4[],timestamptz[],text,text",
		installedBy: "db/migrations/022_screening_ledger_purge_chain_corroboration.sql",
		acceptedBodySHA256: []string{
...
const (
	// ADR-0007 Addendum 13 D116: unchanged by this addendum -- the
	// time-floor overload has no array argument, so D123 row 5 ("checked,
	// and it does not apply") leaves it untouched.
	purgeSnapshotsTimeFloorBodySHA256Migration     = "d44b2cab4d905fafd5754df890b5f95e286cf3877e80cabe4f120bdeb41eea91"
	purgeSnapshotsTimeFloorBodySHA256SchemaSQLBoot = "156cfb60ba5b78c856eb9d893bd65dbee0db3b344ae2c7aba6feb91d39c19534"

	// ADR-0007 Addendum 14 D124/D125: these two moved again, the fourth
	// consecutive round to move them (D107, then D116, now D124) --
	// measured (not guessed) as sha256(prosrc) of db/migrations/024's
	// literal body and SchemaSQL's own copy. D124's fix drives the
	// leading corroboration entirely by cardinality()/unnest(...) WITH
	// ORDINALITY -- no array_length(...,1) and no subscript anywhere in
	// this body -- because the array-form overload's two prior
	// descriptions (array_length(...,1)+[i] vs. = ANY(...)) do not range
	// over the same element set in five of six shapes measured (D123).
	// 023's own array-form literal is now a superseded literal, covered
	// by D99(b)'s already-shipped property without a new pinned
	// constant (D125).
	purgeSnapshotsArrayFormBodySHA256Migration     = "763f63090c9af4be219176d31dd1d7d304d548062533506b85fff9d439a2d0da"
	purgeSnapshotsArrayFormBodySHA256SchemaSQLBoot = "bd7365a49b728621f38d875d345b6fe395f85f09ddd3e31a18c24d5955ba3948"
...
func (p *PostgresSink) checkProvisioningState(ctx context.Context) (ProvisioningState, error) {
	...
	for _, fn := range requiredDefinerFunctions {
		...
		// ADR-0007 Addendum 10 D87/D86 row 8: D77's own reasoning applied
		// one object over -- these two functions are what actually WRITE
		// purged_at, and until this addendum nothing declared their body,
		// so a CREATE OR REPLACE substitution at T2 was reported by
		// neither this check nor grant-ddl-ownership. Joined through the
		// function's own regprocedure OID, exactly as D77 joins through a
		// trigger's tgfoid -- so this follows whatever body is actually
		// live, not whatever the declaring migration says it should be.
		if len(fn.acceptedBodySHA256) > 0 {
			var bodyOK bool
			if err := p.conn.QueryRow(ctx, `SELECT encode(sha256(convert_to(prosrc,'UTF8')),'hex') = ANY($2) FROM pg_proc WHERE oid = $1::regprocedure`, signature, fn.acceptedBodySHA256).Scan(&bodyOK); err != nil {
				return ProvisioningState{}, fmt.Errorf("ADR-0007 Addendum 10 D87: checking function %s body digest: %w", signature, err)
			}
			if !bodyOK {
				return ProvisioningState{Reason: fmt.Sprintf("function %s's body (prosrc) is not in its declared accepted digest set (ADR-0007 Addendum 10 D87): possible CREATE OR REPLACE FUNCTION substitution of a definer function that writes purged_at", signature)}, nil
			}
		}
	}
...
var requiredProtectedObjects = []protectedObjectIdentity{
	...
	{"pg_proc", "public.screening_ledger_purge_snapshots(pg_catalog.text,bigint,timestamp with time zone,pg_catalog.text,pg_catalog.text)"},
	{"pg_proc", "public.screening_ledger_purge_snapshots(pg_catalog.text[],pg_catalog.text,integer[],timestamp with time zone[],pg_catalog.text,pg_catalog.text)"},
...
type requiredProtectedIndexState struct {
	...
	indkey string
	// indnkeyatts is ADR-0007 Addendum 10 D91 (N-C, MEDIUM): indkey
	// renders ALL index attributes, key columns AND INCLUDE columns
	// alike, so "PRIMARY KEY (ledger_id) INCLUDE (sequence)" and
	// "PRIMARY KEY (ledger_id, sequence)" both render indkey="1 2" --
	// indnkeyatts is the only declared property that distinguishes them
	// (the count of KEY columns; INCLUDE columns are payload, not part
	// of what the index enforces uniqueness over). Measured against the
	// baseline: 2 for the anchor pkey (ledger_id, sequence), 1 for the
	// tombstone pkey (snapshot_sha256).
	indnkeyatts int16
}
...
var requiredProtectedRelationStates = []requiredProtectedRelationState{
	{
		...
		indexes: []requiredProtectedIndexState{
			{name: "screening_ledger_anchor_pkey", indisunique: true, indisprimary: true, indkey: "1 2", indnkeyatts: 2},
		},
	},
	{
...
func (p *PostgresSink) protectedRelationStateReason(ctx context.Context) (string, error) {
	for _, want := range requiredProtectedRelationStates {
		...
		for _, idx := range want.indexes {
			var uniqueOK, primaryOK, indkeyOK, indnkeyattsOK, predNullOK, exprsNullOK bool
			err := p.conn.QueryRow(ctx, `
				SELECT
					...
					COALESCE((SELECT ix.indnkeyatts FROM pg_index ix JOIN pg_class c ON c.oid = ix.indexrelid WHERE c.relname = $2 AND ix.indrelid = $1::regclass), -1) = $6,
			...
			`, want.identity, idx.name, idx.indisunique, idx.indisprimary, idx.indkey, idx.indnkeyatts).
				Scan(&uniqueOK, &primaryOK, &indkeyOK, &indnkeyattsOK, &predNullOK, &exprsNullOK)
			if err != nil {
				return "", fmt.Errorf("checking index %s shape on %s (ADR-0007 Addendum 9 D80/Addendum 10 D91): %w", idx.name, want.identity, err)
			}
			switch {
			case !uniqueOK:
			...
			case !indnkeyattsOK:
				return fmt.Sprintf("%s: declared index %s's indnkeyatts does not match the declared %d (ADR-0007 Addendum 10 D91): possible substitution with a weaker index whose key columns are a strict subset of the declared ones (the rest demoted to INCLUDE payload) -- indkey alone cannot see this", want.identity, idx.name, idx.indnkeyatts), nil
			case !predNullOK:
...
func (p *PostgresSink) Persist(ctx context.Context, event Event, request, response SnapshotEnvelope, verification ReplicationVerification) error {
	...
	// ADR-0007 Addendum 12 D105(b): expires_at is bound as a typed
	// time.Time, not a text literal the server parses -- the same
	// convention AnchorSink.WriteAnchor already uses for anchored_at
	// (anchor.go:181-188). A text literal plus ::timestamptz makes the
	// server's own fractional-second parser (round-half-to-even) a
	// SECOND, independent reduction of the value hashEvent's MAC already
	// committed to, which disagrees with the read path's Truncate
	// (anchor.go) on 4,995 of every 9,999 nanosecond values (D104/D105).
	// event.ExpiresAt is already microsecond-precision at creation
	// (D105(a), mustExpires), so this Truncate is a no-op on every value
	// this binary writes; it stays explicit so a chain written by an
	// older binary -- carrying genuine sub-microsecond precision -- still
	// verifies, because both the write path here and the read path
	// (anchor.go) truncate the SAME way rather than one truncating and
	// the other letting the server round.
	expiresAt, err := time.Parse(time.RFC3339Nano, event.ExpiresAt)
	if err != nil {
		return fmt.Errorf("parsing Event.ExpiresAt %q (ADR-0007 Addendum 12 D105(b)): %w", event.ExpiresAt, err)
	}
	...
	if _, err := tx.Exec(ctx,
		`INSERT INTO screening_ledger_event(event_id,ledger_id,sequence,event_sha256,previous_event_sha256,occurred_at,route,http_status,request_sha256,response_sha256,request_snapshot_sha256,response_snapshot_sha256,retention_class,expires_at,event_json)
		 VALUES ($1,$2,$3,$4,$5,$6::timestamptz,$7,$8,$9,$10,$11,$12,$13,$14,$15::jsonb)
		...
		event.RequestSnapshotSHA256, event.ResponseSnapshotSHA256, event.RetentionClass, expiresAt, eventJSON,
	); err != nil {
		return err
	}
...
// extended by Addendum 10 D88(a)): this ledger's anchor row with the
// largest Sequence strictly less than beforeSequence -- ORDER BY
// sequence DESC LIMIT 1 over a WHERE sequence < $2, the same tiebreaker
// shape LatestAnchor uses (anchor_mac DESC), for the same K-F-adjacent
// reason. found=false (pgx.ErrNoRows) means beforeSequence names this
// ledger's first anchor, which is not an error -- D89's genesis case
// (no previousAnchoredAt term) applies there instead. This method does
// not MAC-verify the row it returns -- see AnchorReader's own doc
// comment on why that is the caller's job, not this one's.
func (p *PostgresSink) PreviousAnchorAt(ctx context.Context, ledgerID string, beforeSequence int64) (anchor Anchor, found bool, err error) {
	ctx, cancel := context.WithTimeout(ctx, p.timeout)
	defer cancel()
	var sequence, auditSequence int64
	var eventSHA, auditSHA, policySHA256, mac string
	...
	err = p.conn.QueryRow(ctx,
		`SELECT sequence, event_sha256, audit_sha256, audit_sequence, policy_sha256, anchored_at, anchor_mac
		 FROM screening_ledger_anchor WHERE ledger_id=$1 AND sequence < $2 ORDER BY sequence DESC, anchor_mac DESC LIMIT 1`,
		ledgerID, beforeSequence,
	).Scan(&sequence, &eventSHA, &auditSHA, &auditSequence, &policySHA256, &anchoredAt, &mac)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return Anchor{}, false, nil
		}
...
// ForeignLedgerIDs implements PurgeChecker (ADR-0007 Addendum 12 D110,
// corrected by Addendum 13 D119 in this decision's own words, AR7):
// every DISTINCT ledger_id, other than ledgerID, found in ANY of the
// THREE relations that carry a ledger_id column -- screening_ledger_event
// (008g:3), screening_ledger_audit (008g:9) and screening_ledger_anchor
// (015:23) -- not screening_ledger_event alone. D110's original text
// named all three but the implementation checked only one, so a
// co-tenant with anchor or audit rows and no event rows was structurally
// invisible to the exclusive-tenancy assertion: verified, when the
// schema was not exclusive at all. Each returned entry names the
// relation it was found in ("ledger-id (relation)"), D46's own standard,
// so an operator reading the resulting diagnostic can act on it rather
// than merely being told the schema is shared. Read by owl_migrator with
// no new role, DSN or grant -- SELECT on all three relations is already
// held (screening_ledger_event/audit for the D97/D107 aggregate reads
// above, screening_ledger_anchor from grant-ddl-ownership's own
// SELECT-only grant to owl_migrator), checked rather than assumed, the
// standard D33/D41/D45/D59/D68/D76 each held to.
func (p *PostgresSink) ForeignLedgerIDs(ctx context.Context, ledgerID string) ([]string, error) {
	ctx, cancel := context.WithTimeout(ctx, p.timeout)
	defer cancel()
	rows, err := p.conn.Query(ctx, `
		SELECT ledger_id, 'screening_ledger_event' FROM screening_ledger_event WHERE ledger_id<>$1
		UNION
		SELECT ledger_id, 'screening_ledger_audit' FROM screening_ledger_audit WHERE ledger_id<>$1
		UNION
		SELECT ledger_id, 'screening_ledger_anchor' FROM screening_ledger_anchor WHERE ledger_id<>$1
		ORDER BY 1, 2
	`, ledgerID)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		...
		foreign = append(foreign, fmt.Sprintf("%s (%s)", id, relation))
	}
...
// independent of any local ledger directory or its legal holds. Distinct
// from RecordPurge (D28), which cmd/screening-ledger's purge command
// actually uses: that form takes the caller's already holds-filtered
// candidate set and lets the server re-check the expiry floor against
// exactly those snapshots. PurgeExpired's own eligibility predicate
// (SECURITY DEFINER since 019, ALL-expired-over-the-mirror since 021) is
// unchanged; ADR-0007 Addendum 12 D107 adds a leading refusal, the
// coarsest version of the corroboration RecordPurge below carries per
// snapshot: expectedCount/expectedMax is this ledger's OWN belief about
// its TOTAL chain-authenticated screening_ledger_event population
// (every event this ledger has appended, and the latest ExpiresAt among
// them), refused unless the mirror's own total for ledgerID agrees
// exactly. PurgeExpired has no non-test Go caller (D107's own
// measurement) but must be no weaker than RecordPurge beside it (D76).
// See PersistAudit's doc comment for the bare-statement hazard this
// method also avoids.
func (p *PostgresSink) PurgeExpired(ctx context.Context, ledgerID string, expectedCount int, expectedMax time.Time, operator, reason string) error {
	ctx, cancel := context.WithTimeout(ctx, p.timeout)
	defer cancel()
	...
	// expectedMax binds NULL when expectedCount is 0 -- a ledger with no
	// mirror rows at all has mirror max(expires_at) NULL, and a bound
	// zero-value time.Time would compare IS DISTINCT FROM NULL as true.
	var expectedMaxParam *time.Time
	if expectedCount != 0 {
		expectedMaxParam = &expectedMax
	}
	if _, err := tx.Exec(ctx, `SELECT screening_ledger_purge_snapshots($1,$2,$3,$4,$5)`, ledgerID, int64(expectedCount), expectedMaxParam, operator, reason); err != nil {
		return err
	}
...
// RecordPurge implements PurgeRecorder (ADR-0007 Addendum 2 D27/D28,
// corroboration added by Addendum 12 D107): the local-narrows/
// server-floors purge path. eligibleSHA256 is the set Store.PurgeExpired's
// local pass already determined eligible under this ledger's legal-holds
// rule (holds/, which this call does not and should not learn about);
// obligations is the chain-authenticated (count, MAX) per sha in
// eligibleSHA256 (D107). The array-form overload of
// screening_ledger_purge_snapshots refuses the WHOLE call unless its own
// mirror aggregate agrees with every supplied pair, then re-validates
// eligibility against expires_at server-side and records only what is
// actually expired, regardless of what the caller claims. The caller may
// then mark purged, locally, only the snapshots this call reports as
// recorded.
func (p *PostgresSink) RecordPurge(ctx context.Context, eligibleSHA256 []string, obligations map[string]snapshotObligation, ledgerID, operator, reason string) ([]string, error) {
	ctx, cancel := context.WithTimeout(ctx, p.timeout)
	defer cancel()
	...
	expectedCount := make([]int32, len(eligibleSHA256))
	// expectedMax is *time.Time, nil for a sha with no chain obligation
	// at all (obligations[sha] not present, count 0) -- the mirror's own
	// max(expires_at) over zero matching rows is SQL NULL, and a bound
	// zero-value time.Time would compare IS DISTINCT FROM NULL as true,
	// turning "nothing to purge" into a spurious corroboration refusal.
	expectedMax := make([]*time.Time, len(eligibleSHA256))
	for i, sha := range eligibleSHA256 {
		o, ok := obligations[sha]
		if !ok || o.count == 0 {
			continue
		}
		expectedCount[i] = int32(o.count)
		max := o.max
		expectedMax[i] = &max
	}
	...
	var recorded []string
	if err := tx.QueryRow(ctx,
		`SELECT screening_ledger_purge_snapshots($1::text[],$2,$3::int[],$4::timestamptz[],$5,$6)`,
		eligibleSHA256, ledgerID, expectedCount, expectedMax, operator, reason,
