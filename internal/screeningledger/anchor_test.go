...
// audit_sha256) tuple. A forger who does not hold K_anchor must not be
// able to reproduce anchor_mac by any means that does not depend on it.
func TestAnchorMACDependsOnKey(t *testing.T) {
	keyA := bytes.Repeat([]byte{0xAA}, 32)
	keyB := bytes.Repeat([]byte{0xBB}, 32)

	macA := anchorMAC(keyA, "ledger-anchor", 1, "event-sha", "audit-sha")
	macB := anchorMAC(keyB, "ledger-anchor", 1, "event-sha", "audit-sha")
	if macA == macB {
...
// anchorMAC uses: changing any one of the four inputs it is defined over
// (ADR-0007 §5.3: ledger_id, sequence, event_sha256, audit_sha256) must
// change the output. This is also what would catch a delimiter bug that
// let two different (ledger_id, sequence) pairs collide into the same
// serialized input.
func TestAnchorMACDependsOnEveryField(t *testing.T) {
	key := bytes.Repeat([]byte{0xCC}, 32)
	base := anchorMAC(key, "ledger-a", 1, "event-sha-1", "audit-sha-1")

	variants := map[string]string{
		"ledger_id":    anchorMAC(key, "ledger-b", 1, "event-sha-1", "audit-sha-1"),
		"sequence":     anchorMAC(key, "ledger-a", 2, "event-sha-1", "audit-sha-1"),
		"event_sha256": anchorMAC(key, "ledger-a", 1, "event-sha-2", "audit-sha-1"),
		"audit_sha256": anchorMAC(key, "ledger-a", 1, "event-sha-1", "audit-sha-2"),
	}
...
func TestAnchorMACNoDelimiterCollision(t *testing.T) {
	...
	macA := anchorMAC(key, "ledger-a", 1, "ab", "cd")
	macB := anchorMAC(key, "ledger-a", 1, "abc", "d")
	if macA == macB {
...
// same anchor_mac, since Verify() in a future stage will need to
// recompute it and compare.
func TestAnchorMACIsDeterministic(t *testing.T) {
	key := bytes.Repeat([]byte{0xEE}, 32)
	first := anchorMAC(key, "ledger-a", 7, "event-sha", "audit-sha")
	second := anchorMAC(key, "ledger-a", 7, "event-sha", "audit-sha")
	if first != second {
...
func TestKAnchorDistinctFromChainKeys(t *testing.T) {
	...
	macUnderAnchorKey := anchorMAC(kAnchor, "ledger-anchor", 1, "event-sha", "audit-sha")
	macUnderChainKey := anchorMAC(keys.chain, "ledger-anchor", 1, "event-sha", "audit-sha")
