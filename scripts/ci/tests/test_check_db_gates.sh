...
run_gate() {
  env -u OWL_TEST_DATABASE_URL \
      ...
      -u OWL_MIGRATOR_RESTORED_DATABASE_URL \
      -u OWL_MIGRATOR_CLONED_DATABASE_URL \
      -u OWL_ALLOW_UNPROVEN_DB_GATES \
      "$@" "$GATE_SCRIPT"
}

# --- Case 1: all ten gates unset, no opt-out -> fails closed --------------
set +e
out="$(run_gate 2>&1)"
code=$?
set -e
...
echo "PASS: case 1 (all gates missing, no opt-out -> exit non-zero, all ten named)"

# --- Case 2: all ten gates unset, with opt-out -> fail-open, exit 0 -------
set +e
...
echo "PASS: case 2 (all gates missing, opt-out set -> exit 0, fail-open banner, all ten named)"

# --- Case 3: all ten gates set (bogus but non-empty DSNs) -> passes -------
set +e
out="$(run_gate \
  ...
  OWL_MIGRATOR_RESTORED_DATABASE_URL=postgresql://bogus/db \
  OWL_MIGRATOR_CLONED_DATABASE_URL=postgresql://bogus/db \
  2>&1)"
code=$?
set -e
[[ "$code" -eq 0 ]] || fail "case 3: expected exit 0 with all ten gates set, got $code. Output:\n$out"
[[ "$out" != *"FAIL"* ]] || fail "case 3: expected no FAIL output with all ten gates set. Output:\n$out"
[[ "$out" != *"FAIL-OPEN"* ]] || fail "case 3: expected no fail-open banner with all ten gates set. Output:\n$out"
for gate in "${ALL_GATES[@]}"; do
  [[ "$out" == *"PASS: $gate is set"* ]] || fail "case 3: expected a PASS line naming $gate. Output:\n$out"
done
echo "PASS: case 3 (all gates set -> exit 0, no fail-open banner, all ten named)"
