...
Only the legacy `screeningapiv8d`-`v8g` wrappers remain untested,
deliberately deferred pending the separate #14 keep/archive/delete
decision - adding new tests to code whose retirement is still an open
question isn't a good use of effort, and #14's own resolution explicitly
says to revisit removal rather than keep investing in it indefinitely.
